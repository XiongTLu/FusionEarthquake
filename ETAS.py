"""
ETAS Model Core Module

This script is mainly used to fit ETAS model parameters using maximum likelihood estimation.
It also includes core ETAS functions for intensity calculation and likelihood evaluation.

Includes:
- Intensity function λ(t) for ETAS
- Log-likelihood calculation (total and per-event)
- Magnitude likelihood (Gutenberg-Richter)
...
def estimate_beta_value(mags, threshold):
    # estimate the bate value of the G-R magnitude distribution
    # beta = 1 / mean(m - threshold)
...
def k(m, params, Mcut):
    # expected number of aftershocks triggered by an event of magnitude m
    # applies only if m >= Mcut(minimum triggering magnitude)
...
def f(x, params):
    # omori-type temporal decay kernel
    # f(t) = (omega - 1) * c ^ (omega - 1) / (t + c) ^ omega
    return (params['omega'] - 1) * params['c'] ** (params['omega'] - 1) * 1 / ((x + params['c']) ** params['omega'])

#integrated omori kernel

def H(t, params):
    # integrated omori kernel from 0 to t
    # H(t) = 1 - [c / (t + c)] ^ (omega - 1)
...
def likelihood(Tdat, Mdat, maxtime, params, time_step):
    # computing log-likelihood of the ETAS model given event times and magnitudes
    M0 = params['M0']
    ...
    # subtract background and triggering intgral terms
...
def likelihood_scores(Tdat, Mdat, maxtime, Mcut, M0pred, params, time_step):
    # calculate individual log-likelihood scores for events above M0pred
...
def magnitude_scores(T_test, M_test, Mcut, M0pred, params, time_step):
    # log-likelihood scores based on magnitude distribution (G-R)
...
def intensity(t, hist, params, Mcut):
    # computing total intensity λ(t) at time t
    # includes background rate and all past triggring contributions
    if(t == hist[0][-1]):
        n = len(hist[0]) - 1
...
def datetime64_to_days(dates):
    # convert datetime64 array of float days since the first timestamp
...
def maxlikelihoodETAS(Tdat, Mdat, M0, maxtime=np.nan, initval=np.nan):
    # fit ETAS model parameters using maximum likelihood estimation (MLE)
    if(math.isnan(maxtime)):
        maxtime = Tdat.max()

        beta = 1        # default beta if not estimated
        
    def fn(param_array):
        # innner function to compute negative log-likelihood
        params = {
            'mu' : param_array[0],
            'k0' : param_array[1],
            'a' : param_array[2],
        ...
        if (params['mu'] <= 0 or params['omega'] <= 1 or params['a'] < 0 or params['k0'] < 0): 
            ...
            return(math.inf)    # invalid parameter set
