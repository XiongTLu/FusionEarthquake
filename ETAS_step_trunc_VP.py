...
def etas_time_input(Tdat, Mdat, Mcut, M0pred, params, time_step, Mmax, verbose=True):
    # ---- 读参数 / 常量 ----
    M0   = float(params['M0'])
    mu   = float(params['mu'])
    ...
    Mmax = float(Mmax)
    Z_norm = 1.0 - np.exp(-beta * (Mmax - Mcut))
    pd_tr = (np.exp(-beta * (M0pred - Mcut)) - np.exp(-beta * (Mmax - Mcut))) / Z_norm
    ...
    for i in tqdm(range(1, n), desc='ETAS time LL (vec)', ncols=100):
        ...
        loglikelihood_full[i] = np.log(pd_tr * lam_vals_full[i])   
        ...
        # ===== 累计口径的积分（背景 + 触发）=====
        # 背景累计：pd_tr * mu * (t_i - t_j)
        dt_mu = Tdat[i] - Tdat[j]
        bg_cum = mu * dt_mu
        loglikelihood_full[i] -= pd_tr * bg_cum
        integral_vals_full[i] += bg_cum

        # 触发累计：sum_u k(m_u) [ H(t_i - t_u) - H(t_j - t_u) ]
        ...
        loglikelihood_full[i] -= pd_tr * trig_cum
    ...
    if verbose:
        ...
        for i in range(1, n):
            ...
            if mask_full[i]:  # 段末时刻
                ll_step_recon_full.append(np.log(pd_tr * lam_vals_full[i]) - pd_tr * seg_sum)
                seg_sum = 0.0
...
def etas_mag_input(T_dat, M_dat, Mcut, M0pred, params, time_step, Mmax):
    beta = params['beta']

    Mmax = float(Mmax)
    B = np.exp(-beta * (Mmax - Mcut))
    A = np.exp(-beta * (M0pred - Mcut))
    Z_norm = 1.0 - B

    rate_vals_full = (beta * np.exp(-beta * (M_dat - Mcut))) / Z_norm
    intg_vals_full = (1 - np.exp(-beta * (M_dat - Mcut))) / Z_norm

    mask = (M_dat >= M0pred)
    mags_mask = M_dat[mask]

    likelihood_truncated = (
        np.log(beta) - beta * (mags_mask - Mcut) - np.log(A - B)
    )
