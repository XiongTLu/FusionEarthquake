...
def main():
    ...
    print("\n--- trunc-ETAS VP temporal test set ---")
    ...
    # =========================================================
    #           ETAS-trunc VP 的时间 / 震级 mean LL
    # =========================================================
    time_mean = float(np.mean(test_time_ll_pred))
    mag_mean  = float(np.mean(test_mag_ll_pred))

    print("\n[ETAS-trunc VP mean log-likelihoods | burn-in removed + masked by M>=M0pred]")
    print(f"ETAS-trunc VP test  time      mean LL (masked): {time_mean:.6f}  (n={int(test_time_mask_full.sum())})")
    print(f"ETAS-trunc VP test  magnitude mean LL (masked): {mag_mean:.6f}  (n={int(test_mag_mask_full.sum())})")
