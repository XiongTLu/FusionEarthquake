...
class NPP():
    ...
    def __init__(self, time_step, size_rnn, size_nn, size_layer_chfn, size_layer_cmfn, M0pred):
        """ initilizds key parameters for npp model """
    ...
    def collate_times(self, dT_dat, dM_dat, Mtarget):
        """" generate the target time interval and mask for time arrange"""
        k = 0
    ...
    def set_train_data(self, times, mags):
        """ convert the raw time series into format by the model"""
    ...
    def set_model(self, lam, batch_size=None, stateful=False):
        ...
        ## Outputs
        Int_l = layers.Dense(1,
                             activation='softplus',
                             kernel_initializer=abs_glorot_uniform,
                             kernel_constraint=keras.constraints.NonNeg())(hidden) # cumulative hazard function, positive weights
        ...
        Int_l_mag = layers.Dense(1,
                                 activation='sigmoid',
                                 kernel_initializer=abs_glorot_uniform,
                                 kernel_constraint=keras.constraints.NonNeg())(hidden_mag) # cumulative hazard function, positive weights
    ...
    # def fit_eval(self, epochs=100, batch_size=256, plot_training=False):
    ...
    def save_weights(self, ckp_path, file_name):
        ckp_save = os.path.join(ckp_path, file_name)
        self.model.save_weights(ckp_save)
        return self


    def load_weights(self, ckp_path, file_name):
        tf.compat.v1.logging.set_verbosity(tf.compat.v1.logging.ERROR)

        checkpoint_path = os.path.join(ckp_path, file_name)
    ...
    def predict_eval(self, batch_size=512):
        """ making predictions on the test data and evaluate the model performance """

        [self.lam, self.Int_lam, self.mag_dist, self.Int_mag_dist] = self.model.predict(
        ...
        self.collated_LLmag = (np.log(1e-10 + self.mag_dist))[self.test_mask]
    ...
    def magreshape(self, hist, new_time):
        """ organize historical event data and newly time data for amplitude prediction"""
        T, M = hist
    ...
    def magdensfunc(self, x, reshape_RNN_times, reshape_RNN_mags, reshape_CHFN, reshape_CMFN, mask):
        """ calculate the density function value at a given amplitude """
        reshape_CMFN[-1] = x

        lam, Int_lam, mag_dist, Int_mag_dist = self.model.predict([
            reshape_RNN_times,
            reshape_RNN_mags,
            reshape_CHFN,
            reshape_CMFN,
        ...
        return mag_dist[-1], Int_mag_dist[-1]   
        # return mag_dist[-1]
    
    def timereshape(self, hist, new_mag):
        T, M = hist

        dM_test = np.delete(M, 0)
        dT_test = np.ediff1d(T)
        dM_test = np.append(dM_test, new_mag)
        dT_test = np.append(dT_test, 1e-9)
        
        n = dT_test.shape[0]
        n2 = dM_test.shape[0]

        input_RNN_times = np.array([dT_test[i : i + self.time_step] 
                                   for i in range(n - self.time_step)
                                   ]).reshape(-1, self.time_step, 1)
        input_RNN_mags = np.array([dM_test[i : i + self.time_step]
                                  for i in range(n2 - self.time_step)
                                  ]).reshape(-1, self.time_step, 1)
        input_CHFN = dT_test[-n + self.time_step:].reshape(-1, 1)
        input_CMFN = dM_test[-n + self.time_step:].reshape(-1, 1)
        mask = np.ones_like(input_CHFN)

        return input_RNN_times, input_RNN_mags, input_CHFN, input_CMFN, mask


    

    def timedensfunc(self, x, reshape_RNN_times, reshape_RNN_mags, reshape_CHFN, reshape_CMFN, reshape_mask):
        reshape_CHFN[-1] = x
        # print(reshape_CHFN[-1])

        lam = self.model.predict([
            reshape_RNN_times, 
            reshape_RNN_mags, 
            reshape_CHFN, 
            reshape_CMFN, 
            reshape_mask
        ], batch_size = reshape_RNN_times.shape[0])[0]

        return lam[-1]
                                                                                          

    def magdistfunc(self, x, reshape_RNN_times, reshape_RNN_mags, reshape_CHFN, reshape_CMFN, mask):
        """ calculate the distribution function value at a given amplitude """
        reshape_CMFN[-1] = x

        Int_m_test = self.model.predict([
            reshape_RNN_times, 
            reshape_RNN_mags,
            reshape_CHFN,
            reshape_CMFN,
    ...
    def time_reshape(self, hist):
        """ organize historical event times for time prediction"""
        T, M=hist
        dM_test = np.delete(M, 0)
        dT_test = np.ediff1d(T)
    ...
    def distfunc(self, x, reshape_RNN_times, reshape_RNN_mags, reshape_CHFN, reshape_CMFN, mask):
        """ calculate the distribution function for a time interval"""
        reshape_CHFN[-1] = x

        int_l_test = self.model.predict([
            reshape_RNN_times, 
            reshape_RNN_mags, 
            reshape_CHFN,
            reshape_CMFN, 
        ...
        time_interval_distribution = 1 - np.exp(-int_l_test[-1])
        return time_interval_distribution


    def densfunc(self, x, reshape_RNN_times, reshape_RNN_mags, reshape_CHFN, reshape_CMFN, reshape_mask):
        """ calculate the density function value at a given time interval"""
        reshape_CHFN[-1] = x

        lam, Int_lam = self.model.predict([reshape_RNN_times,
                                           reshape_RNN_mags,
                                           reshape_CHFN,
                                           reshape_CMFN,
                                           reshape_mask
                                           ],batch_size=reshape_mask.shape[0])[:2]
