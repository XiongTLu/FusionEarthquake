...
def main():
    ...
    if use_cache and os.path.exists(etas_train_mag_path):
        ...
        (
         ...
         _,
         train_mag_ll_pred, 
         train_mag_mask_full) = etas_mag_input_pwl(
            T_train, M_train,
    ...
    if use_cache and os.path.exists(etas_test_mag_path):
        ...
        (
         ...
         _,
         test_mag_ll_pred,
         test_mag_mask_full) = etas_mag_input_pwl(
            T_test, M_test,
            Mcut=Mcut, M0pred=M0pred,
            params_3=params,
            time_step=time_step,
    ...
    if use_cache and os.path.exists(etas_train_time_path):
        ...
        (
         ...
         _,
         train_time_ll_pred,
         train_time_mask_full) = etas_time_input_pwl(
            T_train, M_train,
            Mcut=Mcut, M0pred=M0pred,
            params_3=params,
            time_step=time_step,
    ...
    if use_cache and os.path.exists(etas_test_time_path):
        ...
        (
         ...
         _,
         test_time_ll_pred,
         test_time_mask_full) = etas_time_input_pwl(
            T_test, M_test,
            Mcut=Mcut, M0pred=M0pred,
            params_3=params,
            time_step=time_step,
    ...
    # =========================================================
    #           ETAS-PWL fixed 的时间 / 震级 mean LL
    # =========================================================
    time_mean = float(np.mean(test_time_ll_pred))
    mag_mean  = float(np.mean(test_mag_ll_pred))

    print("\n[ETAS-PWL fixed mean log-likelihoods | burn-in removed + masked by M>=M0pred]")
    print(f"ETAS-PWL fixed test time mean LL (masked): {time_mean:.6f}  (n={len(test_time_ll_pred)})")
    print(f"ETAS-PWL fixed test magnitude mean LL (masked): {mag_mean:.6f}  (n={len(test_mag_ll_pred)})")
