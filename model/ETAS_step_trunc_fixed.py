...
def etas_time_input(Tdat, Mdat, Mcut, M0pred, params_3, time_step, Mmax):
    # ---- 读参数 / 常量 ----
    M0   = float(params_3['M0']) # M0 == Mcut
...
def etas_mag_input(T_dat, M_dat, Mcut, M0pred, params_3, time_step, Mmax):
    ...
    likelihood_truncated = (
        np.log(beta_3 * np.exp(-beta_3 * (mags_mask - M0pred))) - np.log(1.0 - np.exp(-beta_3 * (Mmax - M0pred)))
    )
