...
from ETAS_step_PWL_VP import etas_time_input_pwl, etas_mag_input_pwl
...
def main():
    ...
    # =========================================================
    use_cache = True
    etas_input_file_path = f'./etas_inputs_PWL_VP'
    os.makedirs(etas_input_file_path, exist_ok=True)

    # =========================================================
    #                 ETAS-PWL VP: magnitude
    # =========================================================
    print("\n--- ETAS-PWL VP magnitude training set ---")
    etas_train_mag_file = f'ETAS_PWL_VP_train_mag_Mcut{Mcut}_params{VP_params_cut}_{earthquake}.csv'
    ...
    print("\n--- ETAS-PWL VP magnitude test set ---")
    etas_test_mag_file = f'ETAS_PWL_VP_test_mag_Mcut{Mcut}_params{VP_params_cut}_{earthquake}.csv'
    ...
    #                 ETAS-PWL VP: Time
    # =========================================================
    print("\n--- ETAS-PWL VP temporal training set ---")
    etas_train_time_file = f'ETAS_PWL_VP_train_time_Mcut{Mcut}_params{VP_params_cut}_{earthquake}.csv'
    ...
    print("\n--- ETAS-PWL VP temporal test set ---")
    etas_test_time_file = f'ETAS_PWL_VP_test_time_Mcut{Mcut}_params{VP_params_cut}_{earthquake}.csv'
    ...
    if T_train.shape[0] == etas_train_time_rate_full.shape[0]:
        print('\n✅ ETAS PWL VP training set sizes match!')
    ...
    ##################### Fusion-PWL Model #########################
    ckp_dir = './ckp_Fusion_PWL_joint_VP_seed/'
    os.makedirs(ckp_dir, exist_ok=True)

    checkpoint = f'ckp_Fusion_PWL_joint_VP_Mcut{Mcut}_{earthquake}_seed{seed}.h5'
    ...
    # # # # # # # # # # # # #  Testing  # # # # # # # # # # # #
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    result_path = './results_Fusion_PWL_joint_VP_seed/'
    os.makedirs(result_path, exist_ok=True) 
    result_file = f'results_Fusion_PWL_joint_VP_Mcut{Mcut}_{earthquake}_seed{seed}.csv'
    path_to_results = os.path.join(result_path, result_file)

    pred_path = './pred_Fusion_PWL_joint_VP_seed/'
    os.makedirs(pred_path, exist_ok=True)
    pred_file = f'pred_Fusion_PWL_joint_VP_Mcut{Mcut}_{earthquake}_seed{seed}.csv'
    ...
    if not os.path.isfile(path_to_results):
        ...
        print(f'Fusion PWL joint VP Temporal Likelihood: {tf.reduce_mean(model.collated_LLtime).numpy():.4f}')
        print(f'Fusion PWL joint VP Magnitude Likelihood: {tf.reduce_mean(model.collated_LLmag).numpy():.4f}')
        ...
        model.collated_LLtime = LL_results.fusion_PWL_joint_VP_time_like
        print('Fusion PWL joint VP Temporal Likelihood:   ' + str(model.collated_LLtime.mean()))

        model.collated_LLmag = LL_results.fusion_PWL_joint_VP_mag_like
        print('Fusion PWL joint VP Magnitude Likelihood:  ' + str(model.collated_LLmag.mean()))
    ...
    if not os.path.isfile(path_to_results):
        ...
        LL_results ={
            'fusion_PWL_joint_VP_time_like':model.collated_LLtime,
            'fusion_PWL_joint_VP_mag_like':model.collated_LLmag, 
            'LLpoiss': np.repeat(LLpoiss,npredictions),
            'ntrain': np.repeat(T_train.shape[0],npredictions),
        }

        pred_results = {
            'fusion_PWL_joint_VP_temp_rate':rate_time,
            'fusion_PWL_joint_VP_temp_intg':intg_time,
            'fusion_PWL_joint_VP_mag_rate':rate_mag,
            'fusion_PWL_joint_VP_mag_intg':intg_mag,
        }
