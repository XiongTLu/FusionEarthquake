...
from Fusion_PWL_joint import Fusion_PWL_joint
...
def main():
    ...
    ##################### Fusion-PWL Model #########################
    ckp_dir = './ckp_Fusion_PWL_joint_fixed_seed/'
    os.makedirs(ckp_dir, exist_ok=True)

    checkpoint = f'ckp_Fusion_PWL_joint_fixed_Mcut{Mcut}_{earthquake}_seed{seed}.h5'
    ...
    # # # # # # # # # # # # #  Testing  # # # # # # # # # # # #
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    result_path = './results_Fusion_PWL_joint_fixed_seed/'
    os.makedirs(result_path, exist_ok=True) 
    result_file = f'results_Fusion_PWL_joint_fixed_Mcut{Mcut}_{earthquake}_seed{seed}.csv'
    path_to_results = os.path.join(result_path, result_file)

    pred_path = './pred_Fusion_PWL_joint_fixed_seed/'
    os.makedirs(pred_path, exist_ok=True)
    pred_file = f'pred_Fusion_PWL_joint_fixed_Mcut{Mcut}_{earthquake}_seed{seed}.csv'
    ...
    if not os.path.isfile(path_to_results):
        ...
        print(f'Fusion PWL joint fixed Temporal Likelihood: {tf.reduce_mean(model.collated_LLtime).numpy():.4f}')
        print(f'Fusion PWL joint fixed Magnitude Likelihood: {tf.reduce_mean(model.collated_LLmag).numpy():.4f}')
        ...
        model.collated_LLtime = LL_results.fusion_PWL_joint_fixed_time_like
        print('Fusion PWL joint fixed Temporal Likelihood:   ' + str(model.collated_LLtime.mean()))

        model.collated_LLmag = LL_results.fusion_PWL_joint_fixed_mag_like
        print('Fusion PWL joint fixed Magnitude Likelihood:  ' + str(model.collated_LLmag.mean()))
    ...
    if not os.path.isfile(path_to_results):
        ...
        LL_results ={
            'fusion_PWL_joint_fixed_time_like':model.collated_LLtime,
            'fusion_PWL_joint_fixed_mag_like':model.collated_LLmag, 
            'LLpoiss': np.repeat(LLpoiss,npredictions),
            'ntrain': np.repeat(T_train.shape[0],npredictions),
        }

        pred_results = {
            'fusion_PWL_joint_fixed_temp_rate':rate_time,
            'fusion_PWL_joint_fixed_temp_intg':intg_time,
            'fusion_PWL_joint_fixed_mag_rate':rate_mag,
            'fusion_PWL_joint_fixed_mag_intg':intg_mag,
        }
