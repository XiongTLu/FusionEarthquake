...
from ETAS_step_PWL_fixed import etas_time_input_pwl, etas_mag_input_pwl
from Fusion_PWL_score import Fusion_PWL_score
...
def main():
    ...
    # =========================================================
    #                 ETAS-PWL fixed inputs
    # =========================================================
    use_cache = True
    etas_input_file_path = f'./etas_inputs_PWL_fixed'
    os.makedirs(etas_input_file_path, exist_ok=True)

    # =========================================================
    #                 ETAS-PWL fixed: magnitude
    # =========================================================
    print("\n--- ETAS-PWL fixed magnitude training set ---")
    etas_train_mag_file = f'ETAS_PWL_fixed_train_mag_Mcut{Mcut}_params{fixed_params_cut}_{earthquake}.csv'
    ...
    if use_cache and os.path.exists(etas_train_mag_path):
        ...
        (
         ...
         etas_train_mag_mask_full) = etas_mag_input_pwl(
            T_train, M_train,
    ...
    print("\n--- ETAS-PWL fixed magnitude test set ---")
    etas_test_mag_file = f'ETAS_PWL_fixed_test_mag_Mcut{Mcut}_params{fixed_params_cut}_{earthquake}.csv'
    ...
    if use_cache and os.path.exists(etas_test_mag_path):
        ...
        (
         ...
         etas_test_mag_mask_full) = etas_mag_input_pwl(
            T_test, M_test,
            Mcut=Mcut, M0pred=M0pred,
            params_3=params,
            time_step=time_step,
    ...
    #                 ETAS-PWL fixed: Time
    # =========================================================
    print("\n--- ETAS-PWL fixed temporal training set ---")
    etas_train_time_file = f'ETAS_PWL_fixed_train_time_Mcut{Mcut}_params{fixed_params_cut}_{earthquake}.csv'
    ...
    if use_cache and os.path.exists(etas_train_time_path):
        ...
        (
         ...
         etas_train_time_mask_full) = etas_time_input_pwl(
            T_train, M_train,
            Mcut=Mcut, M0pred=M0pred,
            params_3=params,
            time_step=time_step,
    ...
    print("\n--- ETAS-PWL fixed temporal test set ---")
    etas_test_time_file = f'ETAS_PWL_fixed_test_time_Mcut{Mcut}_params{fixed_params_cut}_{earthquake}.csv'
    ...
    if use_cache and os.path.exists(etas_test_time_path):
        ...
        (
         ...
         etas_test_time_mask_full) = etas_time_input_pwl(
            T_test, M_test,
            Mcut=Mcut, M0pred=M0pred,
            params_3=params,
            time_step=time_step,
    ...
    if T_train.shape[0] == etas_train_time_rate_full.shape[0]:
        print('\n✅ ETAS PWL fixed training set sizes match!')
    ...
    ##################### Fusion-PWL Model #########################
    ckp_dir = './ckp_Fusion_PWL_score_fixed_seed/'
    os.makedirs(ckp_dir, exist_ok=True)

    checkpoint = f'ckp_Fusion_PWL_score_fixed_Mcut{Mcut}_{earthquake}_seed{seed}.h5'
    ...
    # # # # # # # # # # # # #  Testing  # # # # # # # # # # # #
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    result_path = './results_Fusion_PWL_score_fixed_seed/'
    os.makedirs(result_path, exist_ok=True) 
    result_file = f'results_Fusion_PWL_score_fixed_Mcut{Mcut}_{earthquake}_seed{seed}.csv'
    path_to_results = os.path.join(result_path, result_file)

    pred_path = './pred_Fusion_PWL_score_fixed_seed/'
    os.makedirs(pred_path, exist_ok=True)
    pred_file = f'pred_Fusion_PWL_score_fixed_Mcut{Mcut}_{earthquake}_seed{seed}.csv'
    ...
    if not os.path.isfile(path_to_results):
        ...
        print(f'Fusion PWL fixed Temporal Likelihood: {tf.reduce_mean(model.collated_LLtime).numpy():.4f}')
        print(f'Fusion PWL fixed Magnitude Likelihood: {tf.reduce_mean(model.collated_LLmag).numpy():.4f}')
        ...
        model.collated_LLtime = LL_results.fusion_PWL_score_fixed_time_like
        print('Fusion PWL score fixed Temporal Likelihood:   ' + str(model.collated_LLtime.mean()))

        model.collated_LLmag = LL_results.fusion_PWL_score_fixed_mag_like
        print('Fusion PWL score fixed Magnitude Likelihood:  '+str(model.collated_LLmag.mean()))
    ...
    if not os.path.isfile(path_to_results):
        ...
        LL_results ={
            'fusion_PWL_score_fixed_time_like':model.collated_LLtime,
            'fusion_PWL_score_fixed_mag_like':model.collated_LLmag, 
            'LLpoiss': np.repeat(LLpoiss,npredictions),
            'ntrain': np.repeat(T_train.shape[0],npredictions),
        }

        pred_results = {
            'fusion_PWL_score_fixed_temp_rate':rate_time,
            'fusion_PWL_score_fixed_temp_intg':intg_time,
            'fusion_PWL_score_fixed_mag_rate':rate_mag,
            'fusion_PWL_score_fixed_mag_intg':intg_mag,
        }
