...
from ETAS_step_trunc_VP import etas_time_input, etas_mag_input
from Fusion_zhang import Fusion_zhang
...
def main():
    ...
    param_path = f'./ETAS_parameters/params_Mcut_{params_cut}_{earthquake}.csv'
    ...
    # =========================================================
    #                 ETAS trunc VP inputs
    # =========================================================
    use_cache = True
    etas_input_file_path = f'./etas_inputs_trunc_VP'
    os.makedirs(etas_input_file_path, exist_ok=True)

    # =========================================================
    #                 ETAS trunc VP: magnitude
    # =========================================================
    print("\n--- ETAS trunc VP magnitude training set ---")
    etas_train_mag_file = f'ETAS_trunc_VP_train_mag_Mcut{Mcut}_params{params_cut}_{earthquake}.csv'
    ...
    print("\n--- ETAS trunc VP magnitude test set ---")
    etas_test_mag_file = f'ETAS_trunc_VP_test_mag_Mcut{Mcut}_params{params_cut}_{earthquake}.csv'
    ...
    #                 ETAS trunc VP: Time
    # =========================================================
    print("\n--- ETAS trunc VP temporal training set ---")
    etas_train_time_file = f'ETAS_trunc_VP_train_time_Mcut{Mcut}_params{params_cut}_{earthquake}.csv'
    ...
    print("\n--- ETAS trunc VP temporal test set ---")
    etas_test_time_file = f'ETAS_trunc_VP_test_time_Mcut{Mcut}_params{params_cut}_{earthquake}.csv'
    ...
    if T_train.shape[0] == etas_train_time_rate_full.shape[0]:
        print('\n✅ ETAS trunc VP training set sizes match!')

    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    # # # # # # # # # # # # #  Training # # # # # # # # # # # # # 
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #  

    ##################### Fusion trunc zhang Model #########################
    ckp_dir = './ckp_Fusion_trunc_VP_zhang_seed/'
    os.makedirs(ckp_dir, exist_ok=True)

    checkpoint = f'ckp_Fusion_trunc_VP_zhang_Mcut{Mcut}_{earthquake}_seed{seed}.h5'
    ...
    if not os.path.isfile(weight_file):
        ...
        model = (
            Fusion_zhang(time_step=time_step,
                   size_rnn=64,
                   size_nn=64,
                   size_layer_chfn=3,
                   size_layer_cmfn=3,
    ...
    # # # # # # # # # # # # #  Testing  # # # # # # # # # # # #
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    result_path = './results_Fusion_trunc_VP_zhang_seed/'
    os.makedirs(result_path, exist_ok=True) 
    result_file = f'results_Fusion_trunc_VP_zhang_Mcut{Mcut}_{earthquake}_seed{seed}.csv'
    path_to_results = os.path.join(result_path, result_file)

    pred_path = './pred_Fusion_trunc_VP_zhang_seed/'
    os.makedirs(pred_path, exist_ok=True)
    pred_file = f'pred_Fusion_trunc_VP_zhang_Mcut{Mcut}_{earthquake}_seed{seed}.csv'
    ...
    if not os.path.isfile(path_to_results):
        ...
        print(f'Fusion trunc VP Temporal Likelihood: {tf.reduce_mean(model.collated_LLtime).numpy():.4f}')
        print(f'Fusion trunc VP Magnitude Likelihood: {tf.reduce_mean(model.collated_LLmag).numpy():.4f}')
        ...
        print('Fusion trunc VP Temporal Likelihood:   ' + str(model.collated_LLtime.mean()))

        model.collated_LLmag = LL_results.Fusion_mag_like
        print('Fusion trunc VP Magnitude Likelihood:  '+str(model.collated_LLmag.mean()))
