...
class NPP_PWL_joint():
    ...
    def __init__(self, time_step, size_rnn, size_nn, size_layer_chfn, size_layer_cmfn, M0pred, Mcut, C_wd, gamma):
    ...
    def set_model(self, lam, batch_size=None, stateful=False):
        ...
        ## define model
        self.model = Model(inputs=[time_history, mag_history, elapsed_time, current_mag, mask], outputs=[l, Int_l, l_mag, Int_l_mag, wm])
        self.model.add_loss(-K.sum((K.log(1e-10 + l) * mask * wm + K.log(1e-10 + l_mag) * mask * wm - Int_l * self.C_wd)))# set loss function to be the negative log-likelihood function
    ...
    def predict_eval(self, batch_size=512):
        ...
        [self.lam, self.Int_lam, self.mag_dist, self.Int_mag_dist, self.wm] = self.model.predict(
        ...
        self.LL = (np.log(self.lam + 1e-10) * self.test_mask * self.wm - self.Int_lam * self.C_wd) 
        self.collated_LLmag = (np.log(1e-10 + self.mag_dist) * self.wm)[self.test_mask]
        self.LL_average = sum(self.LL) / sum(self.test_mask)
        self.collated_LLtime = self.collate_likelihoods(self.LL, self.test_mask)
