...
from NPP_PWL_joint import NPP_PWL_joint
...
def main():
    ...
    ############# Neural Network
    ckp_dir = './ckp_NPP_PWL_joint_fixed_seed/'
    os.makedirs(ckp_dir, exist_ok=True)

    checkpoint = f'ckp_NPP_PWL_joint_fixed_Mcut{Mcut}_{earthquake}_seed{seed}.h5'
    ...
    if not os.path.isfile(weight_file):
        ...
        npp = (NPP_PWL_joint(time_step=time_step,
                  size_rnn=64,
                  size_nn=64,
                  size_layer_chfn=3,
                  size_layer_cmfn=3,
    ...
    # # # # # # # # # # # # #  Testing  # # # # # # # # # # # # # 
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    result_path = './results_NPP_PWL_joint_fixed_seed/'
    os.makedirs(result_path, exist_ok=True)
    result_file = f'results_NPP_PWL_joint_fixed_Mcut{Mcut}_{earthquake}_seed{seed}.csv'
    path_to_results = os.path.join(result_path, result_file)

    pred_path = './pred_NPP_PWL_joint_fixed_seed/'
    os.makedirs(pred_path, exist_ok=True)
    pred_file = f'pred_NPP_PWL_joint_fixed_Mcut{Mcut}_{earthquake}_seed{seed}.csv'
    ...
    ##### NN
        ...
        print('NPP_PWL_joint Temporal Likelihood:   ' + str(npp.collated_LLtime.mean()))
        print('NPP_PWL_joint Mark Likelihood:  '+str(npp.collated_LLmag.mean()))

    else: # read in results

        LL_results = pd.read_csv(path_to_results)

        npp.collated_LLtime = LL_results.NPP_PWL_joint_time_like
        print('NPP_PWL_joint Temporal Likelihood:   ' + str(npp.collated_LLtime.mean()))

        LLpoiss = LL_results.LLpoiss[0]
        print('Poisson Likelihood:   '+str(LLpoiss))

        npp.NPP_mag_like = LL_results.NPP_PWL_joint_mag_like
        print('NPP_PWL_joint Mark Likelihood:  '+str(npp.NPP_mag_like.mean()))
    ...
    # if True:
        ...
        LL_results ={
            'NPP_PWL_joint_time_like':npp.collated_LLtime,
            'NPP_PWL_joint_mag_like':npp.collated_LLmag, 
            'LLpoiss': np.repeat(LLpoiss,npredictions),
            'ntrain': np.repeat(T_train.shape[0],npredictions),
            }
    
        pred_results = {
            'NPP_PWL_joint_temp_rate':lam,
            'NPP_PWL_joint_temp_hazard': Int_lam,
            'NPP_PWL_joint_mag_rate':mag_dist,
            'NPP_PWL_joint_mag_hazard': Int_mag,
        }
