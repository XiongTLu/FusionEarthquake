...
def compute_Fusion_PWL(earthquake, Mcut):
    ...
    path = f'./results_Fusion_PWL_score_fixed_seed'
    results_file_name = (
        f'results_Fusion_PWL_score_fixed_Mcut{Mcut}_{earthquake}_seed{seed}.csv'
    )
    results_path = os.path.join(path, results_file_name)

    results_df = pd.read_csv(results_path)

    time_LL = results_df['fusion_PWL_score_fixed_time_like'].values
    mag_LL = results_df['fusion_PWL_score_fixed_mag_like'].values
...
def main():
    ...
    for earthquake in earthquakes:
        ...
        for Mcut in mcuts:
            ...
            rows.append({
                'Mcut': Mcut,
                f'Fusion_PWL_score_fixed_time_LL': time_LL,
                f'Fusion_PWL_score_fixed_mag_LL': mag_LL,
                'Poisson_LL': poiss_LL,
            })
        ...
        out_path = os.path.join(
            out_dir,
            f"summary_Fusion_PWL_score_fixed_seed1_{earthquake}.csv"
        )
