...
def compute_Fusion_trunc(earthquake, Mcut):
    """
    ...
    path = f'./results_Fusion_trunc_fixed_seed'
    results_file_name = (
        f'results_Fusion_trunc_fixed_Mcut{Mcut}_{earthquake}_seed{seed}.csv'
    )
    results_path = os.path.join(path, results_file_name)

    results_df = pd.read_csv(results_path)

    time_LL = results_df['Fusion_time_like'].values
    mag_LL = results_df['Fusion_mag_like'].values
...
def main():
    ...
    for earthquake in earthquakes:
        ...
        for Mcut in mcuts:
            ...
            time_LL, mag_LL, poiss_LL = compute_Fusion_trunc(
                earthquake,
                Mcut
            )
            ...
            rows.append({
                'Mcut': Mcut,
                f'Fusion_fixed_time_LL': time_LL,
                f'Fusion_fixed_mag_LL': mag_LL,
                'Poisson_LL': poiss_LL,
            })
        ...
        out_path = os.path.join(
            out_dir,
            f"summary_Fusion_fixed_seed1_{earthquake}.csv"
        )
