...
def compute_NPP_PWL(earthquake, Mcut):
    ...
    path = f'./results_NPP_PWL_joint_fixed_seed'
    results_file_name = f'results_NPP_PWL_joint_fixed_Mcut{Mcut}_{earthquake}_seed{seed}.csv'
    results_path = os.path.join(path, results_file_name)

    results_df = pd.read_csv(results_path)

    time_LL = results_df['NPP_PWL_joint_time_like'].values
    mag_LL = results_df['NPP_PWL_joint_mag_like'].values
...
def main():
    ...
    for earthquake in earthquakes:
        ...
        for Mcut in mcuts:
            ...
            rows.append({
                'Mcut': Mcut,
                'NPP_PWL_joint_fixed_time_LL': time_LL,
                'NPP_PWL_joint_fixed_mag_LL': mag_LL,
                'Poisson_LL': poiss_LL,
            })
        ...
        out_path = os.path.join(
            out_dir,
            f"summary_NPP_PWL_joint_fixed_seed1_{earthquake}.csv"
        )
