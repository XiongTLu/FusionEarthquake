...
def compute_etas_trunc_single_event_LL(type_model, earthquake, Mcut, M0pred, Mmax, time_step):
    """
    Compute single-event temporal LL and magnitude LL for each target event,
    ...
    # ----- compute truncated probability term -----
    Delta = Mmax - Mcut
    delta = M0pred - Mcut
    ...
    #   time LL prediction
    # ======================
    time_log_part = np.log(pd_tr * time_rate_full + 1e-10)
    ...
    for earthquake in earthquakes:
        ...
        for Mcut in mcuts:
            ...
            out_path = os.path.join(
                out_dir,
                f'results_ETAS_trunc_VP_Mcut{Mcut:.1f}_{earthquake}.csv'
            )
